...
public:
  ...
  void setIntrinsicParameters(size_t width, size_t height, std::string &cameraName, std::string &distortion_model, std::vector<double> &vD, std::vector<double> &vK, std::vector<double> &vRectMat,
...
public:
  ...
  CameraSystem(const std::string &calibInfoDir, bool left_only, bool bPrintCalibInfo);
  ...
  void loadmplCalibInfo(const std::string &cameraSystemDir, bool bPrintCalibInfo = false);
  void printCalibInfo();
