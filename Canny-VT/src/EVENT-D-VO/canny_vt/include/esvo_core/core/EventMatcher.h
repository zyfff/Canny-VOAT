...
struct EventSlice {
  ...
  EventSlice(double SLICE_THICKNESS = 2 * 1e-3) : SLICE_THICKNESS_(SLICE_THICKNESS) {}
...
public:
  EventMatcher(CameraSystem::Ptr camSysPtr, size_t numThread = 1,
               double Time_THRESHOLD = 10e-5, // 100us
               double EPIPOLAR_THRESHOLD = 0.5, double TS_NCC_THRESHOLD = 0.1, size_t patch_size_X = 25, size_t patch_size_Y = 5, size_t patch_intensity_threshold = 125,
               double patch_valid_ratio = 0.1);
  virtual ~EventMatcher();

  void resetParameters(double Time_THRESHOLD, double EPIPOLAR_THRESHOLD, double TS_NCC_THRESHOLD, size_t patch_size_X, size_t patch_size_Y, size_t patch_intensity_threshold, double patch_valid_ratio);

  void createMatchProblem(StampedTimeSurfaceObs *pTS_obs, std::vector<EventSlice> *vEventSlice_ptr, std::vector<dvs_msgs::Event *> *vEventPtr_cand);

  bool match_an_event(dvs_msgs::Event *ev_ptr, Transformation &Trans_world_rv, EventMatchPair &emPair);
  ...
  bool warping2(const Eigen::Vector2d &x, double invDepth, const Eigen::Matrix<double, 3, 4> &T_left_rv, Eigen::Vector2d &x1_s, Eigen::Vector2d &x2_s);

  bool patchInterpolation2(const Eigen::MatrixXd &img, const Eigen::Vector2d &location, Eigen::MatrixXd &patch);
