...
template <class T> class SmartGrid {
public:
...
template <class T> SmartGrid<T>::SmartGrid() = default;

template <class T> SmartGrid<T>::SmartGrid(size_t rows, size_t cols) {
  _grid.reserve(rows);
...
template <class T> SmartGrid<T>::~SmartGrid() {
  for (size_t r = 0; r < rows(); r++)
    delete _grid[r];
}

template <class T> SmartGrid<T> &SmartGrid<T>::operator=(const SmartGrid &rhs) {
  // copy grid elements
...
template <class T> void SmartGrid<T>::resize(size_t rows, size_t cols) {
  this->reset();
  _grid.reserve(rows);
  for (size_t r = 0; r < rows; r++) {
    _grid.push_back(new std::vector<T>());
...
template <class T> void SmartGrid<T>::erode(size_t radius, size_t border, double ratio) {
...
template <class T> void SmartGrid<T>::dilate(size_t radius) {
  // have to save a temporary copy of the depth map
  SmartGrid gridTmp(rows(), cols());
...
template <class T> void SmartGrid<T>::clean(double var_threshold, double age_threshold, double range_max, double range_min) {
...
template <class T> void SmartGrid<T>::clear() {
  typename gridElements::iterator it = _elements.begin();
  while (it != _elements.end()) {
    typename gridElements::iterator temp = it;
    it++;
...
template <class T> void SmartGrid<T>::reset() {
  for (size_t r = 0; r < rows(); r++) {
...
template <class T> const T &SmartGrid<T>::operator()(size_t row, size_t col) const {
  if ((*_grid[row])[col] != NULL)
    return *((*_grid[row])[col]);
  return _invalid;
}

template <class T> const T &SmartGrid<T>::at(size_t row, size_t col) const {
  if ((*_grid[row])[col] != NULL)
    return *((*_grid[row])[col]);
  return _invalid;
}

template <class T> size_t SmartGrid<T>::rows() const { return _grid.size(); }

template <class T> size_t SmartGrid<T>::cols() const { return (*_grid.front()).size(); }

template <class T> void SmartGrid<T>::set(size_t row, size_t col, const T &value) {
  if ((*_grid[row])[col] == NULL) {
...
template <class T> bool SmartGrid<T>::exists(size_t row, size_t col) {
  if ((*_grid[row])[col] == NULL)
    return false;
  return true;
}

template <class T> T &SmartGrid<T>::get(size_t row, size_t col) {
  if ((*_grid[row])[col] == NULL)
    return _invalid;
  return *((*_grid[row])[col]);
}

template <class T> typename SmartGrid<T>::gridElements::iterator SmartGrid<T>::remove(typename SmartGrid<T>::gridElements::iterator &it) {
  size_t row = it->row();
  size_t col = it->col();
  (*_grid[row])[col] = NULL;
...
template <class T> typename SmartGrid<T>::gridElements::iterator SmartGrid<T>::begin() { return _elements.begin(); }

template <class T> typename SmartGrid<T>::gridElements::iterator SmartGrid<T>::end() { return _elements.end(); }

template <class T> size_t SmartGrid<T>::size() const { return _elements.size(); }

template <class T> void SmartGrid<T>::getNeighbourhood(size_t row, size_t col, size_t radius, std::vector<T *> &neighbours) {
