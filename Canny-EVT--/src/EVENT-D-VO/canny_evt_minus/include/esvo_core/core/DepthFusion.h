...
public:
  ...
  DepthFusion(CameraSystem::Ptr &camSysPtr, std::shared_ptr<DepthProblemConfig> &dpConfigPtr);

  virtual ~DepthFusion();

  bool propagate_one_point(DepthPoint &dp_prior, DepthPoint &dp_prop, Eigen::Matrix<double, 4, 4> &T_prop_prior);

  int fusion(DepthPoint &dp_prop, DepthMap::Ptr &dm, int fusion_radius = 0);

  int update(std::vector<DepthPoint> &dp_obs, DepthFrame::Ptr &df, int fusion_radius = 0);
  ...
  void naive_propagation(std::vector<DepthPoint> &dp_obs, DepthFrame::Ptr &df);

  bool naive_propagate_one_point(DepthPoint &dp_prior, DepthPoint &dp_prop, Eigen::Matrix<double, 4, 4> &T_prop_prior);
