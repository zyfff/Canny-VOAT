...
enum DepthProblemType { ANALYTICAL, NUMERICAL };
...
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  DepthProblemSolver(CameraSystem::Ptr &camSysPtr, std::shared_ptr<DepthProblemConfig> &dpConfigPtr,
  ...
  void solve(std::vector<EventMatchPair> *pvEMP, StampedTimeSurfaceObs *pStampedTsObs, std::vector<DepthPoint> &vdp);

  void solve_multiple_problems(Job &job);

  bool solve_single_problem_numerical(double d_init, std::shared_ptr<Eigen::NumericalDiff<DepthProblem>> &dProblemPtr, double *result);

  void pointCulling(std::vector<DepthPoint> &vdp, double std_variance_threshold, double cost_threshold, double invDepth_min_range, double invDepth_max_range);
