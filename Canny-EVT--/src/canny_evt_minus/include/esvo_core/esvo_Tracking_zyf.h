...
enum TrackingStatus { IDLE, WORKING };
...
public:
  ...
  void choosekf(RefFrame ref, CurFrame cur);
  bool boundaryCheck(double xcoor, double ycoor, size_t width, size_t height);
  int count_event_num(TimeSurfaceObservation &ts_ob);
  ...
  void depthCallback(const sensor_msgs::PointCloud2::ConstPtr &pc_msg, const geometry_msgs::PoseStampedConstPtr &pose_msg, const sensor_msgs::ImageConstPtr &mask);
  // results
...
private:
  ...
  message_filters::Subscriber<sensor_msgs::PointCloud2> depth_pc_sub_;
  message_filters::Subscriber<geometry_msgs::PoseStamped> depth_pose_sub_;
  message_filters::Subscriber<sensor_msgs::Image> mask_sub_;
  // message_filters::Subscriber<sensor_msgs::PointCloud2, geometry_msgs::PoseStamped> depth_pc_sub_, depth_pose_sub_;
  ...
  // depth pose and pc sync policy
  typedef message_filters::sync_policies::ExactTime<sensor_msgs::PointCloud2, geometry_msgs::PoseStamped, sensor_msgs::Image> ExactDepthPolicy;
  message_filters::Synchronizer<ExactDepthPolicy> Depth_sync_;
  ...
  RefPointCloudMap2 refPCMap2_; // added by zyf. Include mask.
  cv::Mat mask_depth_;
  // RefPointCloudPoseMap refPCPMap_;
  RefFrame ref_;
  CurFrame cur_;

  TimeSurfaceObservation TS_temp_;
