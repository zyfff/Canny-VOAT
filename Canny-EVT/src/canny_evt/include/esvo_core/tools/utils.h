...
using RefPointCloudMap2 = std::map<ros::Time, cv_bridge::CvImagePtr>; // added by zyf

using Transformation = kindr::minimal::QuatTransformation;

inline static std::vector<dvs_msgs::Event *>::iterator EventVecPtr_lower_bound(std::vector<dvs_msgs::Event *> &vEventPtr, ros::Time &t) {
  return std::lower_bound(vEventPtr.begin(), vEventPtr.end(), t, [](const dvs_msgs::Event *e, const ros::Time &t) { return e->ts.toSec() < t.toSec(); });
}
...
inline static EventQueue::iterator EventBuffer_lower_bound(EventQueue &eb, ros::Time &t) {
  return std::lower_bound(eb.begin(), eb.end(), t, [](const dvs_msgs::Event &e, const ros::Time &t) { return e.ts.toSec() < t.toSec(); });
}

inline static EventQueue::iterator EventBuffer_upper_bound(EventQueue &eb, ros::Time &t) {
  return std::upper_bound(eb.begin(), eb.end(), t, [](const ros::Time &t, const dvs_msgs::Event &e) { return t.toSec() < e.ts.toSec(); });
}

using StampTransformationMap = std::map<ros::Time, tools::Transformation>;
inline static StampTransformationMap::iterator StampTransformationMap_lower_bound(StampTransformationMap &stm, ros::Time &t) {
  return std::lower_bound(stm.begin(), stm.end(), t, [](const std::pair<ros::Time, tools::Transformation> &st, const ros::Time &t) { return st.first.toSec() < t.toSec(); });
}
