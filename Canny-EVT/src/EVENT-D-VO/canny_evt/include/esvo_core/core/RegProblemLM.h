...
struct RegProblemConfig {
  ...
  RegProblemConfig(size_t patchSize_X, size_t patchSize_Y, size_t kernelSize, const std::string &LSnorm, double huber_threshold, double invDepth_min_range = 0.2, double invDepth_max_range = 2.0,
                   const size_t MIN_NUM_EVENTS = 1000, const size_t MAX_REGISTRATION_POINTS = 500, const size_t BATCH_SIZE = 200, const size_t MAX_ITERATION = 10)
      : patchSize_X_(patchSize_X), patchSize_Y_(patchSize_Y), kernelSize_(kernelSize), LSnorm_(LSnorm), huber_threshold_(huber_threshold), invDepth_min_range_(invDepth_min_range),
        invDepth_max_range_(invDepth_max_range), MIN_NUM_EVENTS_(MIN_NUM_EVENTS), MAX_REGISTRATION_POINTS_(MAX_REGISTRATION_POINTS), BATCH_SIZE_(BATCH_SIZE), MAX_ITERATION_(MAX_ITERATION) {}
...
struct RegProblemLM : public optimization::OptimizationFunctor<double> {
  ...
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  RegProblemLM(const CameraSystem::Ptr &camSysPtr, const RegProblemConfig::Ptr &rpConfig_ptr, size_t numThread = 1);
  void setProblem(RefFrame *ref, CurFrame *cur, bool bComputeGrad = false);
  void setProblem(RefFrame *ref, CurFrame *cur, Eigen::Vector3d V, Eigen::Vector3d W, Transformation PredictPose, bool bComputeGrad = false);//revised by zzz
  ...
  //added by zzz
  std::tuple<double, Eigen::Vector2d, Eigen::Vector2d, int> CalNormalFlowWeight(const Eigen::Vector3d &p, const Eigen::Vector3d &pg, const Eigen::Matrix4d &warpingTransf, const Eigen::Matrix4d &warpingTransf1);
  double CalNormalFlowWeight1(const Eigen::Vector3d &p, const Eigen::Matrix4d &warpingTransf) const;
  
  bool reprojection(const Eigen::Vector3d &p, const Eigen::Matrix4d &warpingTransf, Eigen::Vector2d &x1_s) const;

  bool patchInterpolation(const Eigen::MatrixXd &img, const Eigen::Vector2d &location, Eigen::MatrixXd &patch, bool debug = false) const;
  //
  ...
  Eigen::Matrix<double, 4, 4> T_world_pre_;  // added by zzz
  ...
  int positiveNum = 0;     //统计不同TSM的数量
  int negativeNum = 0;
  int allNum = 0;
  int counter_ = 0;     //统计第几次优化，前面几次不用annf
