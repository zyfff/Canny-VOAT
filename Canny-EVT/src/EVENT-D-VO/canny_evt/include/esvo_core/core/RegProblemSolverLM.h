...
public:
  ...
  bool resetRegProblem(RefFrame *ref, CurFrame *cur, Eigen::Vector3d V, Eigen::Vector3d W, Transformation PredictPose);//added by zzz
  bool solve_numerical();  // relatively slower
  bool solve_analytical(); // faster

  // For test and visualization
  void setRegPublisher(image_transport::Publisher *reprojMap_pub, image_transport::Publisher *DistanceField_pub);
...
private:
  ...
  image_transport::Publisher *reprojMap_pub_, *DistanceField_pub_;
