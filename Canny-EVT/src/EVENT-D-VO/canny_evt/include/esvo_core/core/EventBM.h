...
public:
  EventBM(CameraSystem::Ptr camSysPtr, size_t numThread = 1, bool bSmoothTS = false, size_t patch_size_X = 25, size_t patch_size_Y = 25, size_t min_disparity = 1, size_t max_disparity = 40,
          size_t step = 1, double ZNCC_Threshold = 0.1,
          bool bUpDownConfiguration = false); // bUpDownConfiguration determines the epipolar searching direction (UpDown or LeftRight).
  virtual ~EventBM();

  void resetParameters(size_t patch_size_X, size_t patch_size_Y, size_t min_disparity, size_t max_disparity, size_t step, double ZNCC_Threshold, bool bDonwUpConfiguration);

  void createMatchProblem(StampedTimeSurfaceObs *pStampedTsObs, StampTransformationMap *pSt_map, std::vector<dvs_msgs::Event *> *pvEventsPtr);

  bool match_an_event(dvs_msgs::Event *pEvent, std::pair<size_t, size_t> &pDisparityBound, EventMatchPair &emPair);
...
private:
  ...
  bool epipolarSearching(double &min_cost, Eigen::Vector2i &bestMatch, size_t &bestDisp, Eigen::MatrixXd &patch_dst, size_t searching_start_pos, size_t searching_end_pos, size_t searching_step,
